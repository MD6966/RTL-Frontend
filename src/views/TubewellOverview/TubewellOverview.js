...
import { get_Tw_sensor } from 'store/actions';
...
const TubewellOverview = () => {
  const classes = useStyles();
  const dispatch = useDispatch();
  const tubewell = useSelector((state) => state.tw.tubewell);
  const user_id = useSelector((state) => state.auth.user.id);
  const isLoading = useSelector((state) => state.tw.generalLoading);
  ...
  useEffect(() => {
    if (index === 0) {
      dispatch(get_Tw_sensor(user_id));
    }
...
export default TubewellOverview;
