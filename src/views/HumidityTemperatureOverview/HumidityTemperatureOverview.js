...
import { getHumiditySensors } from 'store/actions/HumidityAction';
...
const TemperatureOverview = () => {
  ...
  const sensor = useSelector((state) => state.humidity.humidity);
  ...
  useEffect(() => {
    if (index === 0) {
      dispatch(getHumiditySensors(user_id));
    }
  ...
  return (
    <Page
      ...
      title="Humidity and Temperature Monitoring System"
    >
      {blocked ? (
        <>
          <img
