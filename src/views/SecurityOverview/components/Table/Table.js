...
const Table = (props) => {
  ...
  const [tableType, setTableType] = useState('door_status');
  const [startDate, setStartDate] = useState(moment().subtract(1, 'days'));
  const [endDate, setEndDate] = useState(moment());
  const [selectEdge, setSelectEdge] = useState(null);
  const [calendarDate, setCalendarDate] = useState(moment());
  const data = useSelector((state) => state.security.security[index].logs.data);
  const title = useSelector((state) => state.security.security[index].logs.title);
  const columns = useSelector((state) => state.security.security[index].logs.columns);
  const isLoading = useSelector(
    (state) => state.security.security[index].logs.isLoading
  );
