...
import { get_Rc_sensor } from 'store/actions';
...
const RectifierOverview = () => {
  const classes = useStyles();
  const dispatch = useDispatch();
  const rectifier = useSelector((state) => state.rectifier.rectifier);
  const user_id = useSelector((state) => state.auth.user.id);
  const isLoading = useSelector((state) => state.rectifier.generalLoading);
  ...
  useEffect(() => {
    if (index === 0) {
      dispatch(get_Rc_sensor(user_id));
    }
...
export default RectifierOverview;
