...
import { get_Tank_sensor } from 'store/actions/wtAction';
...
const WtOverview = () => {
  ...
  const isLoading = useSelector((state) => state.wt.isLoading);
  ...
  useEffect(() => {
    if (index === 0) {
      dispatch(get_Tank_sensor(user_id));
    }
