...
import { getTableData } from 'store/actions/lightAction';
...
const Table = (props) => {
  ...
  const [tableType, setTableType] = useState('radar_enable');
  const [startDate, setStartDate] = useState(moment().subtract(1, 'days'));
  const [endDate, setEndDate] = useState(moment());
  const [selectEdge, setSelectEdge] = useState(null);
  const [calendarDate, setCalendarDate] = useState(moment());
  const data = useSelector((state) => state.hl.light[index].logs.data);
  const title = useSelector((state) => state.hl.light[index].logs.title);
  const columns = useSelector((state) => state.hl.light[index].logs.columns);
  const isLoading = useSelector(
    (state) => state.hl.light[index].logs.isLoading
  );
      ...
      >
        ...
        >
          ...
          <Select
            ...
            value={tableType}
          >
            <MenuItem value="radar_enable">Radar Mode Logs</MenuItem>
            <MenuItem value="segControl">Segment Control Logs</MenuItem>
            <MenuItem value="light_time">Timer Logs</MenuItem>
          </Select>
        </FormControl>
      </Grid>
      <Grid
